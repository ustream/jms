...
module.exports = function (request, response) {
	...
	modulerequest(request, function (err, modulereq) {


		if (err) {
			return responseHandler(request, response, modulereq, err);
		}
		...
		if (modulereq.healthcheck) {
			statistics.out();
			return statusResponse(request, response, modulereq);
		}

		if (modulereq.redirect) {
			return redirectResponse(request, response, modulereq);
		}
