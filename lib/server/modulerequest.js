...
// de ezt az osszes included es excluded keresre
	...
	storage.hmget('map', modulelist, function (err, result) {
...
function ModuleRequest (req, done) {
	...
	if (url.pathname == '/client.js') {
		request.client = true;
		storage.hmget('map', ['jmsclient'], function (err, result) {
			request.include.push(result[0]);
			return done(null, request);
		});
		return;
	}
	...
	if (netConf.cache) {
		storage.hmget('cache', [request.href], ifRequestExists.bind(null, done, request));
	} else {
		setupRequest(done, request);
	}
