...
function setupRequest (originalRequest, httpResponse, moduleRequest, done) {
	...
	//moduleRequest.sourceMapRequest = path.indexOf('.js.map') > -1;


	if (moduleRequest.include.length < 1) {
		return done(new Exception400(moduleRequest.pathname), originalRequest, httpResponse, moduleRequest);
	}
...
function ModuleRequest (originalRequest, httpResponse, done) {
	...
	// ha ures az include, akkor is 415
