...
function loadModules (modulereq, done) {
	...
	if (modulereq.timer) {
		modulereq.timer.loadingModules = +new Date();
	}
	...
	if (modulereq.client) {
		...
		storage.get('buildNumber', function (err, number) {
			storage.get(number + ':jmsclient', function (err, module) {
				if (module === null) {
					done(new Exception404('client'));
					return;
				}
				done(null, modulereq, [JSON.parse(module)]);
			});
		});
		return;
	}
