...
var start = function (err) {
	...
	if (err) {
		log.error('clusters', 'error occured during build');
		setupShutdown();
		return;
	}
...
////////////////////
	...
	cluster.on('fork', function(worker) {
		log.verbose('clusters', "worker " + worker.process.pid + " (#"+worker.id+") has spawned");
	});

	cluster.on('listening', function(worker, address) {
		log.verbose('clusters', "worker " + worker.process.pid + " (#"+worker.id+") is listening");
	});

	cluster.on('exit', function(worker, code, signal) {
		log.warn('clusters', "worker " + worker.process.pid + " (#"+worker.id+") has exited");
