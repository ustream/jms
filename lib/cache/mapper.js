...
var cacheAlias = require('./cache-alias');
...
module.exports = function(modules, callback) {
	...
	modules[config.init_app].transitive_dependencies.map(function (dep) {
		modules[dep].source = modules[dep].source.replace(new RegExp("JSMS_cacheMap",'g'), aliasName);
	})

	modules[config.init_app].source = modules[config.init_app].source.replace(new RegExp("JSMS_cacheMap",'g'), aliasName);

	modules[config.init_app].transitive_dependencies.unshift(alias);
